...
const DEFAULT_SITE_CONTENT = {
    ...
    settings: {
        ...
        email: 'croping@gmail.com',
        phone: '+91 94121 62807',
        facebookUrl: '',
        twitterUrl: '',
        instagramUrl: '',
        youtubeUrl: '',
...
// ─────────────────────────────────────────────────────────────────────────────
async function getOrCreateSection(sectionKey) {
...
router.put('/content/:sectionKey', adminAuthMiddleware, async (req, res) => {
    try {
        const { sectionKey } = req.params;
        const { data } = req.body;
        if (data === undefined) {
        ...
        const section = await SiteContent.findOneAndUpdate({ sectionKey }, { data }, { new: true, upsert: true });
        return res.status(200).json({ success: true, data: section.data });
    }
    catch (error) {
...
router.post('/upload-image', adminAuthMiddleware, upload.single('image'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const result = await new Promise((resolve, reject) => {
            cloudinary.uploader.upload_stream({ folder: 'gurukulam-ngo', resource_type: 'image' }, (error, result) => {
                if (error)
                    reject(error);
                else
                    resolve(result);
...
router.post('/gallery', adminAuthMiddleware, upload.single('image'), async (req, res) => {
    try {
        ...
        if (req.file && !finalUrl) {
            ...
            if (cloudName && apiKey && apiSecret) {
                const result = await new Promise((resolve, reject) => {
                    cloudinary.uploader.upload_stream({ folder: 'gurukulam-ngo/gallery', resource_type: 'image' }, (error, result) => {
                        if (error)
                            reject(error);
                        else
                            resolve(result);
...
router.put('/gallery/:id', adminAuthMiddleware, async (req, res) => {
    try {
        const { captionEn, captionHi, captionGu, section } = req.body;
        const image = await GalleryImage.findByIdAndUpdate(req.params.id, { captionEn, captionHi, captionGu, section }, { new: true });
        if (!image)
...
router.delete('/gallery/:id', adminAuthMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
