...
router.post('/', authenticate, authorize(['super-admin', 'admin']), async (req, res) => {
    try {
...
// @access  Super-Admin/Admin
router.put('/:id', authenticate, authorize(['super-admin', 'admin']), async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { $set: { role, status } }, { new: true }).select('-passwordHash');
        if (!user)
