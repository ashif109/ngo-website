...
// @desc    Upload media
// @access  Admin/Editor
router.post('/upload', authenticate, authorize(['super-admin', 'admin', 'content-manager', 'editor']), upload.single('file'), async (req: any, res: any) => {
  try {
    if (!req.file) {
...
// @desc    Delete media
// @access  Admin only
router.delete('/:id', authenticate, authorize(['super-admin', 'admin', 'content-manager']), async (req: any, res: any) => {
  try {
