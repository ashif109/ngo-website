...
const ContactPage: React.FC = () => {
  ...
  return (
    <>
      ...
      <main className="flex-grow bg-primary-dark text-white py-16 relative overflow-hidden" id="contact-page-content">
        ...
        <div className="institutional-container relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 mb-20">
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-white/[0.02] border border-white/5 p-6 sm:p-8 rounded-sm space-y-4">
                <div className="flex gap-4 items-center">
                  <div className="w-10 h-10 rounded-full bg-primary-light/10 border border-secondary/20 flex items-center justify-center text-secondary">
                    <Mail size={16} />
                  </div>
                  <div>
                    <h5 className="text-[10px] font-black uppercase text-secondary-light tracking-wider">{t('contact.emailLabel')}</h5>
                    <p className="text-sm font-bold text-white mt-0.5">croping@gmail.com</p>
                  </div>
                </div>

                <div className="flex gap-4 items-center">
                  <div className="w-10 h-10 rounded-full bg-accent/10 border border-secondary/20 flex items-center justify-center text-orange-400">
                    <Phone size={16} />
                  </div>
                  <div>
                    <h5 className="text-[10px] font-black uppercase text-secondary-light tracking-wider">{t('contact.phoneLabel')}</h5>
              ...
              <div className="bg-white/[0.02] border border-white/5 p-6 sm:p-8 rounded-sm space-y-4">
                ...
                <div className="flex gap-4">
                  <a href="#" className="w-10 h-10 bg-primary flex items-center justify-center rounded-full hover:bg-primary-dark transition-colors font-black text-sm">f</a>
            ...
            <div className="lg:col-span-7 bg-white/5 backdrop-blur-md border border-white/10 rounded-sm p-8 sm:p-10 shadow-2xl relative border-t-4 border-secondary overflow-hidden">
              {status === 'success' ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-[10px] font-black text-secondary-light uppercase tracking-wider mb-2">{t('contact.formName')}</label>
                      <input 
                        required 
                        type="text" 
                        value={formData.name} 
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-secondary-light uppercase tracking-wider mb-2">{t('contact.formEmail')}</label>
                      <input 
                        required 
                        type="email" 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-[10px] font-black text-secondary-light uppercase tracking-wider mb-2">{t('contact.formPhone')}</label>
                      <input 
                        required 
                        type="tel" 
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-secondary-light uppercase tracking-wider mb-2">{t('contact.formSubject')}</label>
                      <input 
                        required 
                        type="text" 
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-secondary-light uppercase tracking-wider mb-2">{t('contact.formMessage')}</label>
                    <textarea 
                      required
                      rows={5} 
          ...
          <div className="mb-20">
            ...
            <div className="w-full h-[400px] bg-primary-dark/40 border border-white/10 rounded-sm overflow-hidden shadow-2xl relative">
              <iframe 
                ...
                <div key={idx} className="border border-white/10 rounded-sm bg-white/[0.02] overflow-hidden">
                  ...
                  <AnimatePresence initial={false}>
                    {activeFaq === idx && (
                      ...
                      >
                        <p className="p-5 pt-0 text-xs sm:text-sm text-secondary-light/80 leading-relaxed border-t border-white/5">
                          {faq.a}
                        </p>
                      </motion.div>
                    )}
